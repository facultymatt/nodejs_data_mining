...
exports.searchAddonsAndUpdateDatabase = function(successCb, failureCb, alphaString) {
  ...
  async.applyEachSeries([
    ...
    saveStats,
    loadNeverFetchedRepos,
    fetchDetails,
    updateRepos,
    fetchCommentsForEach
...
function fetchCommentsForEach(processer, callback) {
  async.each(processer.addons, function(item, eachCallback) {

    // this method will fetch and save automatically
    // we don't need to worry about a callback
    item.fetchRecentCommits(function() {
      eachCallback(null);
    }, function(err) {
      eachCallback(err);
    });
...
function loadNeverFetchedRepos(processer, callback) {
  Addon.find({
    lastFetchedAt: null
  }).exec(function(err, allAddons) {
    if (err) return callback(err);
