...
exports.getDetailsForRepoArrayAsync = function(repoArrayOrObj, detailsSuccess, detailsFailure) {
  ...
  var q = async.queue(function(task, doneCallback) {
    ...
    async.waterfall([
      function(callback) {
        getDetailsForRepo(task.full_name, function(err, result, fetchTime) {
          if (err) return detailsFailure(task.full_name + ' was not found on github');
          callback(null, result, fetchTime);
        });
      },
      function(contents, fetchTime, callback) {
        githubUtil.parseRepoContentsAsync(contents, function(result) {
          callback(null, result, fetchTime);
        });
      },
      function(result, fetchTime) {
        doneCallback(result, fetchTime);
      }
    ]);
  ...
  async.each(repoArrayOrObj, function(repo) {
    q.push({
      full_name: repo.full_name || repo.fullName || null
    }, function(loadedRepo, fetchTime) {
      repo = _.assign(repo, loadedRepo); // @todo make async? 
      repo.lastFetchedAt = fetchTime;
      return repo;
    });
  });
...
function getDetailsForRepo(full_name, callback) {
  ...
  client.repo(full_name).contents('', function(err, body, header) {
    ...
    // success
      ...
      var fetchTime = new Date(header.date).toISOString();
      return callback(null, body, fetchTime);
    }
