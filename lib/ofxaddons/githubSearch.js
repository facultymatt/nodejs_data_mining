...
//
exports.findAllReposAsync = function(req, res) {
...
// @param {string} String of letters to search for, useful for testing
//

exports.searchAlphabetAysnc = function(searchSuccess, searchFailure, alphabetString) {
  ...
  alphabetString = alphabetString || 'abcdefghijklmnopqrstuvwxyz';
  ...
  var q = async.queue(function(task, callback) {
    ...
    var onSuccess = function(allItemsAndStats) {
      ...
      var parsedItems = [];
      async.each(allItemsAndStats.items, function(item, eachCb) {
        //parsedItems.push(githubUtil.parseRepo(item));
        eachCb();
      }, function() {
        // replace items with our parsed items
        //allItemsAndStats.items = parsedItems;
...
function handleReponse(err, body, header, query, itemsAndStats, successCb, failureCb) {
  ...
  if (err) {
    ...
    if (err.statusCode === 403) {
      ...
      return githubUtil.callAfterGivenTime(resetTime, getNextPage, [query, itemsAndStats, successCb, failureCb]);
    }
  ...
  // callback after each runs to parse the repos
  var returnOrGetNextPage = function() {
    ...
    if (body.items &&
      body.items.length >= query.per_page &&
      itemsAndStats.items.length < body.total_count) {
  ...
  // parse and callback
  async.each(body.items, function(item, eachCb) {
    // add any data from the request here
    item.fetchTime = new Date(header.date).toISOString();
    // push the parsed item to our collection
    itemsAndStats.items.push(githubUtil.parseRepo(item));
    eachCb();
  }, returnOrGetNextPage);
