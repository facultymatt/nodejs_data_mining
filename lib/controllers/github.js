...
function handleReponse(err, body, header, query, itemsCollection, successCb, failureCb) {
  ...
  if (err) {
    ...
    if (err.statusCode === 403) {
      ...
      return repeatAfterRateLimitReset(err.headers['x-ratelimit-reset'], query, itemsCollection, successCb, failureCb);
    } else {
...
function repeatAfterRateLimitReset(resetTime, query, itemsCollection, successCb, failureCb) {
